...
if(!file_exists('foto_usuario/'.$id)){
?>
	<form id="form" method="POST" action="td.php" enctype="multipart/form-data">
...
?>
<p class="titulo" id="dados">Os Defensores</p>
	<div>
	<div class="align"  >
	<h3>Ficha Técnica</h3>
Criado por	Douglas Petrie, Marco Ramirez (2017)<br><br>
Com	Charlie Cox, Krysten Ritter, Mike Colter <br><br>
País	EUA<br><br>
Gênero	Fantasia, Ação<br><br>
Status	Em andamento
	</div>
	<img src="./img/td.jpg" class="img" class="align"/>
	</div>
<p class="titulo2" id="sinopse"> Sinopse </p>
<p class="sinopse">A minissérie reúne Matt Murdock (Charlie Cox), o Demolidor; Jessica Jones (Krysten Ritter), Luke Cage (Mike Colter) e Danny Rand (Finn Jones), o Punho de Ferro, e conta a história dos 'heróis imperfeitos de Hell's Kitchen'. </p>
<p class="titulo2" id="trailer"> Trailer</p>
<iframe src="https://www.youtube.com/embed/tuPGY7Md6nA" class="video"></iframe>
...
<div class="comentar" id="comentar">
	<h3 id="perfil" style="margin-left:  26vw"><?php if($c == false){ echo "Digite aqui seu comentário"; } else { echo "!Só é permitido um comentário do usuário por filme!";} ?></h3>
		<form method="POST" action="td.php" style="margin-left:  26vw" style="visibility: <?php if($c == true){ echo "hidden"; } ?>">
...
<section>
	<?php
	$select = mysqli_query($conexao,'SELECT c.idFilme, c.idUsuario, c.coment,c.stars, u.nome FROM comentar c JOIN usuario u ON c.idUsuario = u.id WHERE c.idFilme =23');
