...
<head>
	...
	<link rel="stylesheet" type="text/css" media="(max-width: 769px)" href="./css/RgeralID.css"/>s
...
if(!file_exists('foto_usuario/'.$id)){
?>
	<form id="form" method="POST" action="gg.php" enctype="multipart/form-data">
...
?>
<p class="titulo" id="dados">Guardiões da Galáxia</p>
	<div>
	<div class="align"  >
	<h3>Ficha Técnica</h3>
	Data de lançamento 31 de julho de 2014 <br><br>
Direção: James Gunn (II)<br><br>
Elenco: Chris Pratt, Zoe Saldana, Dave Bautista <br><br>
Gêneros Ficção científica, Ação<br><br>
Nacionalidades EUA, Reino Unido
	</div>
	<img src="./img/gg.jpeg" class="img" class="align"/>
	</div>
...
<iframe src="https://www.youtube.com/embed/6FVulnc4poc" class="video"></iframe>
...
<div class="comentar" id="comentar">
	<h3 id="perfil" style="margin-left:  26vw"><?php if($c == false){ echo "Digite aqui seu comentário"; } else { echo "!Só é permitido um comentário do usuário por filme!";} ?></h3>
		<form method="POST" action="gg.php" style="margin-left:  26vw" style="visibility: <?php if($c == true){ echo "hidden"; } ?>">
...
<section>
	<?php
	$select = mysqli_query($conexao,'SELECT c.idFilme, c.idUsuario, c.coment,c.stars, u.nome FROM comentar c JOIN usuario u ON c.idUsuario = u.id WHERE c.idFilme =7');
